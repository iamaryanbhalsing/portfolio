...
export function MusicPlayer() {
  ...
  const currentTrackRef = useRef<Track>(playlist[0]);
  const trackIndexRef = useRef(0);
  const volumeRef = useRef(70);
  const isMutedRef = useRef(false);
  const isYouTube = !!currentTrack.videoId;
          ...
          onStateChange: (e: { data: number }) => {
            if (e.data === 0) {
              ...
              if (track.videoId && ytPlayerRef.current) {
                ...
                musicGenerator.play(track.preset);
  ...
  // Sync audioState with generator for non-YouTube tracks
  useEffect(() => {
    const interval = setInterval(() => {
      if (!isYouTube && musicGenerator.getIsPlaying()) {
        const analyser = musicGenerator.getAnalyser();
        if (analyser) {
          const data = new Uint8Array(analyser.frequencyBinCount);
          analyser.getByteFrequencyData(data);
          const bass =
            (data[2] + data[3] + data[4]) / (3 * 255);
          const mid =
            (data[10] + data[15] + data[20]) / (3 * 255);
          const treble =
            (data[40] + data[50] + data[60]) / (3 * 255);
  ...
  const prevTrack = useCallback(() => {
    const prev =
      trackIndex === 0 ? playlist.length - 1 : trackIndex - 1;
  ...
  }, [isPlaying, currentTrack, elapsed, ensureInit, isYouTube]);
    ...
    const id = setInterval(() => {
      if (isYouTube) {
        ...
        setBars((prev) =>
          prev.map((_, i) => {
            if (i < 8)
              return bass * (0.4 + Math.random() * 0.6);
            if (i < 16)
              return mid * (0.3 + Math.random() * 0.7);
  ...
  return (
    <>
      ...
      <button
        ...
        aria-label={
          isOpen ? "Close music player" : "Open music player"
        }
      >
        {isOpen ? (
                ...
                <div className="w-full text-center mb-3">
                  <div className="marquee-container">
                    <h3 className="text-sm font-semibold text-white/90 marquee-text">
                      {currentTrack.title}
                      <span className="mx-8 text-white/20">
                        •
                      </span>
                      {currentTrack.title}
                    </h3>
